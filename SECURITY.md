...
- npm releases are staged, never published directly: CI packs, then stages via OIDC trusted publishing (`pnpm stage publish` lives only in the release workflow, not in published package.json scripts). Drydock reviews the exact staged artifact, and a maintainer promotes it with 2FA. There are no npm tokens.
