...
describe("NatsTaskProvider", () => {
	let provider: NatsTaskProvider;
	const testQueue = uniqueQueue();

	// Track custom providers for cleanup
	const customProviders: NatsTaskProvider[] = [];

	async function createCustomProvider(
		options: ConstructorParameters<typeof NatsTaskProvider>[0] = {},
	): Promise<NatsTaskProvider> {
		const p = new NatsTaskProvider(options);
	...
	describe("hookified inheritance", () => {
		...
		test("should emit error events when operations fail in ack", async () => {
	...
	describe("constructor and initialization", () => {
		test("should initialize with default values", () => {
			const p = new NatsTaskProvider();
			expect(p.id).toBe(defaultNatsTaskId);
		...
		test("should fail to connect when NATS is not available", async () => {
			const p = new NatsTaskProvider({ uri: "localhost:9999" });
	...
	describe("getDeadLetterTasks", () => {
		...
		test("should return empty array when DLQ stream does not exist", async () => {
			const deadLetterTasks = await provider.getDeadLetterTasks(
				"nonexistent-dlq-queue-xyz",
			);
			expect(deadLetterTasks).toEqual([]);
		});
	...
	describe("getQueueStats", () => {
		test("should return empty stats for non-existent queue", async () => {
			const stats = await provider.getQueueStats("nonexistent-queue-xyz");
